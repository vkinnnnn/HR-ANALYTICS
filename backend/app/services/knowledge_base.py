...
def _get_collection():
    ...
    if _collection is None:
        try:
            ...
            logger.info(f"Knowledge base collection initialized at {settings.CHROMA_PERSIST_DIR}")
        except Exception as e:
            logger.error(f"Failed to initialize knowledge base collection: {e}")
            raise
    return _collection
...
def rebuild_knowledge_base(data_cache: Dict[str, Any]) -> int:
    ...
    try:
        ...
        if emp_df is not None and len(emp_df) > 0:
            ...
            avg_tenure = emp_df[emp_df["is_active"]]["tenure_years"].mean() if len(emp_df[emp_df["is_active"]]) > 0 else 0
            overview = f"Workforce: {len(emp_df)} employees ({int(active)} active), avg tenure {avg_tenure:.1f}yr, {emp_df['department_name'].nunique()} departments"
...
def search(query: str, n_results: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        if not query or not isinstance(query, str):
            logger.warning("Invalid query for knowledge base search")
            return []
        ...
        logger.debug(f"Knowledge base search returned {len(output)} results for query: {query[:50]}")
        return output
    except Exception as e:
